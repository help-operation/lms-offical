...
import { Mail, Send, XCircle, Calendar, Clock } from "lucide-react";
import { dashboardApi, type CommunicationOverview } from "../api";

export function EmailOverviewCard() {
  const [data, setData] = useState<CommunicationOverview["email"] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    dashboardApi
      .communication()
      .then((res) => setData(res.data.email))
      .catch(() => setData(null))
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm p-5 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-28 h-28 bg-gradient-to-bl from-blue-50/60 to-transparent rounded-bl-full dark:from-blue-500/5" />
      <div className="flex items-center gap-2 mb-4 relative z-10">
        <div className="h-5 w-1 rounded-full bg-blue-500" />
        <span className="text-xs font-bold text-blue-600 dark:text-blue uppercase tracking-wider">Email Overview</span>
