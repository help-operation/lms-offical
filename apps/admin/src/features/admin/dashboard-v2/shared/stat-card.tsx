...
export function StatCard({ card }: { card: StatCardDef }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 mb-0.5 min-w-0 relative z-10">
        <div className={`inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${card.iconBg} dark:bg-white/10`}>
          <card.icon className={`h-3 w-3 ${card.iconColor} dark:text-slate-300`} />
        </div>
        <p className="text-[10px] font-medium text-gray-500 dark:text-slate-400 truncate">{card.label}</p>
      </div>
