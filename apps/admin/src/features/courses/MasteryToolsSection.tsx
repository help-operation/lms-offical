...
export function MasteryToolsSection({ items, title }: { items?: ToolInfoItem[]; title?: string }) {
  const list = items && items.length > 0 ? items : DEFAULT_ITEMS;

  return (
    <div className="mx-auto max-w-[1160px] px-[10px] py-8">
      <style>{`
        @keyframes draw {
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {list.map((item, index) => (
          <div
            key={index}
            className={`${item.bgColor} rounded-xl flex flex-col items-center justify-center`}
            style={{ width: 270, height: 115 }}
          >
            {item.image ? (
              <div style={{ width: 248, height: 62 }} className="flex items-center justify-center">
            ...
            <div style={{ width: 248, height: 30 }} className="flex items-center justify-center">
              <span className="text-white text-center" style={{ fontFamily: '"Hind Siliguri", sans-serif', fontSize: 30, fontWeight: 600 }}>{item.name}</span>
            </div>
          </div>
        ))}
      </div>
