        ...
        <div className="flex items-center gap-1">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star
              key={i}
              className={`h-4 w-4 ${
                i < c.rating ? "fill-yellow-400 text-yellow-400" : "text-gray-300 dark:text-gray-600"
              }`}
            />
          ))}
...
export function TopCoursesCarousel({ courses }: { courses: LandingCourse[] }) {
  const trackRef = useRef<HTMLDivElement>(null);
  const [active, setActive] = useState(0);
  const [paused, setPaused] = useState(false);
  const n = courses.length;
  // Track renders the list twice back-to-back so autoplay/next can keep
  // scrolling forward forever — once it drifts into the duplicated half we
  // silently snap back to the equivalent real position (see effect below),
  // so it never visibly reverses direction.
