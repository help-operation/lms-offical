...
export const metadata = { title: "Return Policy | LearnHub" };

export default async function ReturnPolicyPage() {
  const page = await getCachedPage("return-policy");

  return (
    <main className="mx-auto max-w-3xl px-6 py-16 bg-white dark:bg-gray-950 min-h-screen">
      <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-8">{page?.title ?? "Return Policy"}</h1>
      {page?.content ? (
        <div
