-- NOTE: this migration was extended to also create the shop_* tables/types.
-- The migration tracker in this environment had migration 0023 (which
-- originally created them) marked as already-applied even though the tables
-- were never actually present in this database (a pre-existing tracker/DB
-- drift, unrelated to the display_invoice_number column below). Since 0023
-- can't be re-run (its hash is already recorded), the missing DDL is caught
-- up here instead, ahead of the ALTER TABLE that depends on shop_orders existing.
...
ALTER TABLE "shop_products" ADD CONSTRAINT "shop_products_course_id_courses_id_fk" FOREIGN KEY ("course_id") REFERENCES "public"."courses"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "shop_orders" ADD COLUMN "display_invoice_number" varchar(40);--> statement-breakpoint
ALTER TABLE "shop_orders" ADD CONSTRAINT "shop_orders_display_invoice_number_unique" UNIQUE("display_invoice_number");
